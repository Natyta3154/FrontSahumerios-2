...
export default function AdminAttributesPage() {
  ...
  // --- Ya no necesitamos token, la cookie se envía automáticamente ---
  const fetchAttributes = async () => {
    try {
      const fetchedAttributes = await getAttributes(); // quitar token
      setAttributes(fetchedAttributes);
    } catch (err: any) {
      toast({
        title: "Error",
        description: err.message || "No se pudieron cargar los atributos",
        variant: "destructive",
      });
    }
  };
  ...
  const handleDelete = (attributeId: number) => {
    startTransition(async () => {
      const result = await deleteAttribute(attributeId); // quitar token
      if (result?.error) {
        toast({
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground">
            Gestiona los atributos de los productos, como marcas o materiales.
          </p>
        </div>
